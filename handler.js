...
let chat = global.db.data.chats[m.chat]
let user = global.db.data.users[m.sender]
if (!['gc-unbanchat.js'].includes(name) && chat && chat.isBanned && !isROwner) return // Except this
...
if (name != 'gc-unbanchat.js' && chat?.isBanned)
return 
...
conn.reply(m.chat, `Se agotaron tus *🤍 diamantes*`, m, fake)
continue
}
let extra = {
match,
...
if (m.diamantes)
conn.reply(m.chat, `Utilizaste *${+m.diamantes}* 🤍`, m)
}
break
}}
} catch (e) {
