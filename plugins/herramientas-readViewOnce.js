...
let handler = async (m, { conn }) => {
    if (!m.quoted) return conn.reply(m.chat, `*[ ℹ️ ] Responde a un mensaje ViewOnce (ver solo una vez)*.`, m);
    if (!m.quoted.viewOnce) return conn.reply(m.chat, `*[ ℹ️ ] Responde a un mensaje ViewOnce (ver solo una vez)*`, m);
