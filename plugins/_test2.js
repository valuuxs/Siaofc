...
handler.all = async function (m, { conn }) {
    ...
    if (m.isBot || m.id.startsWith('NJX-')) return;
    if (m.fromMe) return;

    let chat = global.db.data.chats[m.chat];
    if (!chat || chat.isBanned) return;

    // Ignorar mensajes de usuarios que contengan "bot"
    if (m.sender.toLowerCase().includes('bot')) return;

    const name = await conn.getName(m.sender);
    const user = `@${m.sender.split`@`[0]}`;
    const text = m.text?.toLowerCase() || '';

    // Palabras clave para activar IA
    const iaTriggers = ['bot', 'china', 'simsimi', 'simi', 'alexa'];
    const excludeWords = ['jadibot', 'bots', 'serbot', 'instalarbot', 'infobot'];

    if (iaTriggers.some(w => text.includes(w)) && !excludeWords.some(w => text.includes(w))) {
        try {
            await conn.sendPresenceUpdate('composing', m.chat);

            const luminAI = async (q, username, logic) => {
                try {
            ...
            const prompt = `Actualmente juegas el rol de una chica llamada 𝕮𝖍𝖎𝖓𝖆 💋...`; // (puedes resumir o dejar igual)
            let result = await luminAI(m.text, username, prompt);
            ...
            try {
                const gpt = await fetch(`https://api.example.com/tools/simi?text=${encodeURIComponent(m.text)}`);
                const res = await gpt.json();
                return m.reply(res.data.message);
            } catch (e) {
                const fallbackMessages = [
                    `Simsimi está durmiendo, no molestes 🥱`,
                    `Cállate`,
                    `API Simsimi caída`,
                    `Simsimi está ocupado, vuelve más tarde 🥵`,
                    `No molestes, por favor`,
                    `Sin señal`,
                    `No estoy disponible en este momento`
                ];
                console.error("Error en todas las APIs:", e);
                return m.reply(pickRandom(fallbackMessages));
            }
        }
    }

    if (/^@51927238856$/i.test(m.text)) {
        return conn.reply(m.chat, `*No molestes oe duerme mejor*`, m);
    }

    if (/^(mande )?porno$|paja$/i.test(text)) {
        const teks = pickRandom([
            ` 𝑨𝒔𝒊́ 𝒒𝒖𝒆 𝒒𝒖𝒊𝒆𝒓𝒂 𝒉𝒂𝒈𝒂 𝒑𝒖𝒕𝒊𝒕𝒐 🧐`,
            `_uff mire un pajero_`,
            `_pagame y paso mi pack😏🥵_`,
            `_que_`,
            `_que quiere pija dice 🤣`,
            `_pasa el pack de tu hermana😏_`,
            `_mire un gilipolla_`,
            `_callate putito_`
        ]);
        return conn.reply(m.chat, teks, m, { mentions: { mentionedJid: [m.sender] } });
    }

    if (/quiero un bot|necesito un bot|solicit[ao] bot/i.test(text)) {
...
*¿Cómo puedo pagar?*
A través de Paypal, Mercado Pago & Yape:

💸 *PayPal:* https://paypal.me/CristianYTEscobar  
🇦🇷 *Mercado Pago:* Camila Ledesma | CVU: 0000077200132501513706  
🇵🇪 *Yape:* 927238856

⏩ Envía el comprobante para activar tu bot: wa.me/51927238856  
https://chat.whatsapp.com/FCS6htvAmlT7nq006lxU4I

> 「 Cristian Escobar 」`, m);
    }

    if (/¿?qué es un bot\??/i.test(text)) {
        return conn.reply(m.chat, `\`☆::¿𝙌𝙐𝙀 𝙀𝙎 𝙐𝙉 𝘽𝙊𝙏 𝘿𝙀 𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋?::☆\`

> Un bot es una inteligencia artificial que ejecuta tareas automáticas mediante comandos.
> Puede crear stickers, descargar música/videos, logos y más.
> Para ver los comandos disponibles, usa *#menu*.
