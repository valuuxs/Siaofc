...
handler.all = async function (m) {
    ...
    let iaTriggers = ['bot', 'china', 'simsimi', 'simi', 'alexa'];
    let excludeWords = ['jadibot', 'bots', 'serbot', 'instalarbot', 'infobot'];

    // Solo palabra exacta (respuesta aleatoria)
    if (iaTriggers.some(w => text === w) && !excludeWords.some(w => text.includes(w))) {
        return await m.reply(pickRandom([
    ...
    // Palabra + texto (respuesta con IA)
    if (iaTriggers.some(w => text.includes(w)) && !excludeWords.some(w => text.includes(w))) {
        try {
            ...
            let query = m.text;
            let username = `${m.pushName}`;
            let syms1 = `Actualmente juegas el rol de una chica llamada 𝕮𝖍𝖎𝖓𝖆 💋...`; // (puedes dejar tu prompt completo aquí)
    ...
    // Otros gatillos que ya tenías
    if (/^@51927238856$/i.test(m.text)) {
        conn.reply(m.chat, `*No molestes oe duerme mejor*`, m);
    }

    if (/^Mande porno|porno|paja$/i.test(m.text)) {
        let teks = `${pickRandom([
    ...
    if (/^Quiero un bot|como obtengo un bot?|solicit[oa] bot|Necesito un bot$/i.test(m.text)) {
        conn.reply(m.chat, `*\`⚡ ¿Quieres un bot para tu grupo?\`*\n\n...`, m, {
            contextInfo: {
                externalAdReply: {
                    title: `Hola ${name} 👋`,
